  ...
  return `
    <div class="ui-overlay-backdrop">
      <section class="ui-modal intro-modal" role="dialog" aria-modal="true" aria-labelledby="intro-title">
        ...
        <p>迷ったら「ヒントを見る」や「今見る場所」を確認してください。</p>
        ...
        <div class="modal-actions">
          <button type="button" class="action-btn success" data-action="dismiss_intro">はじめる</button>
          <button type="button" class="action-btn secondary" data-action="intro_open_stage_select">ステージを選ぶ</button>
        </div>
      </section>
    </div>
  `
...
function stageSelectButtonHtml(stage) {
  ...
  const completionClass = label === "クリア済み" ? "complete" : "pending"
  return `
    <button
      type="button"
      class="stage-select-card ${completionClass} ${isCurrent ? "current" : ""}"
      data-action="jump_stage"
      data-value="${escapeHtml(stage.id)}"
      ${isCurrent ? 'aria-current="step"' : ""}
    >
      <span class="stage-select-card-head">
      ...
      <span class="stage-select-chapter">${escapeHtml(stage.chapterTitle)}</span>
      <span class="stage-select-goal">${escapeHtml(stage.goal)}</span>
      <span class="stage-select-focus">注目: ${escapeHtml(stage.focus)}</span>
    </button>
  `
}
...
function stageGroupHtml(group) {
  return `
    <section class="stage-select-group">
    ...
    <div class="ui-overlay-backdrop">
      <section class="ui-modal stage-select-modal" role="dialog" aria-modal="true" aria-labelledby="stage-select-title">
        <div class="modal-head">
          ...
          <button type="button" class="action-btn ghost" data-action="close_overlay">閉じる</button>
        </div>
        <div class="stage-select-groups">
          ${groupedStages().map(stageGroupHtml).join("")}
        </div>
      </section>
    </div>
  `
...
function renderOverlay() {
  ...
  root.hidden = false
  root.innerHTML =
    runtime.activeOverlay === "stage_select" ? stageSelectOverlayHtml() : introOverlayHtml()
}
...
function buttonsHtml(stage, state) {
  return [...actionConfig(stage, state), ...utilityActionConfig(state)]
    .map(
      (button) => `
        <button
          type="button"
          class="action-btn ${escapeHtml(button.kind)}"
          data-action="${escapeHtml(button.action)}"
          ${button.value ? `data-value="${escapeHtml(button.value)}"` : ""}
...
function utilityActionConfig(state) {
  return [
...
function applyAction(action, value = "") {
  ...
  if (action === "toggle_stage_hint") {
    state.manualHintShown = !state.manualHintShown
    appState.stageStates[stage.id] = mergeStageState(stage.id, state)
    saveProgress()
    render()
    return
  }
  ...
  nextState.manualHintShown = Boolean(state.manualHintShown)
